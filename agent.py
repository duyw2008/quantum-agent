...
""")
    ...
    def _init_readline(self):
        ...
        self._completions = [
            ...
            'WaveGrid', 'PotentialBuilder', 'gaussian_wavepacket', 'evolve_ssfm', 'animate_wave',
            'PauliX', 'PauliY', 'PauliZ', 'qubit', 'bloch_dm', 'bloch_len', 'rx', 'ry', 'rz', 'hadamard', 'cnot',
...
r'\\sqrt\{([^{}]*(?:\{[^{}]*\}[^{}]*)*?)\}',
    ...
    def calc(self, expr: str):
        ...
        ns = {
            ...
            'PotentialBuilder': qm.PotentialBuilder,
            'tensor': qm.tensor, 'partial_trace': qm.partial_trace,
            'entropy_vn': qm.entropy_vn, 'concurrence': qm.concurrence,
            'bell_states': qm.bell_states,
            'PauliX': qm.sigma_x(), 'PauliY': qm.sigma_y(), 'PauliZ': qm.sigma_z(),
            'qubit': qm.qubit_state, 'bloch_dm': qm.bloch_vector_to_dm, 'bloch_vec': qm.dm_to_bloch_vector, 'bloch_len': qm.bloch_length, 'bloch_length': qm.bloch_length,
            'rx': qm.rotation_x, 'ry': qm.rotation_y, 'rz': qm.rotation_z,
            'hadamard': qm.hadamard, 'cnot': qm.cnot,
            'WaveGrid2D': qm.WaveGrid2D, 'gaussian_beam': qm.gaussian_beam,
            'evolve_ssfm_2d': qm.evolve_ssfm_2d,
    ...
    def _function_help(self, name):
        ...
        formulas_latex = {
            ...
            'potentialbuilder': r'V = PotentialBuilder(grid).harmonic(1).barrier(3,5,0.5).build()',
            'entropy_vn': r'S(\rho) = -\mathrm{Tr}[\rho\log_2\rho]',
            'concurrence': r'C(|\psi\rangle) = 2|\alpha\delta - \beta\gamma|',
